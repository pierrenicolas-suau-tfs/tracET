...
```
  * If is not do it automatically, add to your bashrc:
```commandline
export PATH=$PATH:~/.local/bin
```

  * Ensure all the requirements are install. Maybe you need to install manually some of them, specially *open3d* 
...
### Saliency map:
* Script description:
  * The name of the script is `get_saliency.py`. We can call it with the command:
```commandline
get_saliency - options
```
  * From a tomogram with a binary segmentation, calculates the saliency map, a distance transformation of the input softed with a gaussian filter.
...
* Parameters:
  ...
  * The parameter **soomth_desviation**, called with `-s` or `-sdesv`, is the deviation for gaussian filter and it should be ~1/3 of the element radium.

* Outputs:
  * A **tomogram with the saliency map**, in the same format of the input and with the same name with the suffix *_saliency.mrc* or *_saliency.nrrd*.

### Non-Maximum Suppression:
* Script description:
  * The name of the script is `apply_nonmaxsup.py`. We can call it with the command:
```commandline
apply_nonmaxsup - options
```
  * From a segmentation or a saliency map, detect the most central voxels of the elements and construct an equispatial point cloud of this elements. 
...
* Parameters:
  ...
  * The parameter **filter**, called with `-f` or "--filt", is the filter for the mask of the suppression. It is **optional** and if is not given, only eliminate negative values.
  * The parameter **downsample**, called with `-d` or `--downs`, It is **optional** and if is given, apply a downsample of the radius indicated.
  

* Outputs:
  * A **tomogram with only the maximums of the saliency map**, in the same format of the input and with the same name with the sufix _*supred.mrc* or *_supred.nrrd*

### Spatial Embedded Graph
* Script description:
  * The name of the script is `trace_graph.py`. We can call it with the command:
```commandline
trace_graph - options
```
...
* Parameters:
  ...
  * The parameter **subsampling**, called with `-s` or `--subsam`, is the radius used for subsample points. It is **optional** and if is not given, there is not subsampling.
...
* Outputs:
  ...
  * A **csv file** with the information of the graph components, branches and geometric data, with the same name of the input and the extension *_skel_graph.csv*

### Blobs Clustering
* Script description:
  * The name of the script is `Get_cluster.py`. We can call it with the command:
```commandline
get_cluster - options
```
...
* Parameters:
  ...
  * The parameter **mode**, called with `-m` or `--mode`, is the parameter to select the algorithm of clustering:
    * If is `Affinity`, **Affinity propagation algorithm** is used. Is only recommended for small tomograms.
...
* Script description:
  * The name of the script is `membrane_poly.py`. We can call it with the command:
```commandline
...
* Parameters:
  ...
  * The parameter **min_samples**, called with `-s` or `--samp`, is the minimum samples needed to make a cluster. It is **optional** and if is not given, it takes value 2.
...
* Script description:
  * The name of the script is `seg_skel_dice.py`. We can call it with the command:
```commandline
...
* Outputs:
  ...
  * **TP metric** value.
  * **DICE metric** value.
  * (Optional) Asked with `-o` or `-otomo`, **skeleton of the input tomogram** in *mrc* or *nrrd* format (The same of the input).
  * (Optional) Asked with `-t` or `-ogt`, **skeleton of the ground truth tomogram** in *mrc* or *nrrd* format (The same of the input).
...
In general, the scheme follows the next flux graph, but we usually skip the saliency because is also include in the non-maximum suppression. 

![tutorial_flux](images_tutorial/tutorial_scheme.png)

In three sections we will describe how to get results for every kind of structure.

**DISCLAIMER: This software is made thinking in big tomograms. This may fail if you try with tomograms of less than 100 pixels of thickness in the Z axis**
...
We can see the graph, prioritizing the connections and the curves prioritizing the curvature.