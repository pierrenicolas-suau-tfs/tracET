...
class Segment(core.DeepFinder):
    def __init__(self, Ncl, path_weights, patch_size=192, model_func='deep_finder'):
        """
        Constructor
        :param Ncl: Number of classes (only works for classifications models)
        :param path_weights: path for the trained weights
        :param patch_size: number of pixel for the processing patch
        :param model_func: model function name (Default deep_finder)
        """
        ...
        if hasattr(models, self.model_func):
            if self.model_func == 'light_3D_unet_segmentation':
                self.net = models.light_3D_unet_segmentation(self.P, self.Ncl)
            elif self.model_func == 'light_3D_unet_regression':
                self.net = models.light_3D_unet_regression(self.P)
            elif self.model_func == 'deep_finder':
                self.net = models.deep_finder(self.P, self.Ncl)
            elif self.model_func == 'deep_finder_regression':
                self.net = models.deep_finder_regression(self.P)
            else:
    ...
    def launch(self, dataArray):
        ...
        l = int(self.P / 2)
        lcrop = int(l - self.pcrop)
        step = int(2 * l + 1 - self.poverlap)
        ...
        if 'regression' in self.model_func:
            predArray = np.zeros(dim, dtype=np.float16)
        else:
            ...
            for y in pcenterY:
                for z in pcenterZ:
                    ...
                    predArray[x - lcrop:x + lcrop, y - lcrop:y + lcrop, z - lcrop:z + lcrop] = predArray[
                                                                                                  x - lcrop:x + lcrop,
                                                                                                  y - lcrop:y + lcrop,
                                                                                                  z - lcrop:z + lcrop] + np.float16(
                        pred[0,
                        l - lcrop:l + lcrop,
                        l - lcrop:l + lcrop,
                        l - lcrop:l + lcrop, 0])
                    if not ('regression' in self.model_func):
                        predArray[x - lcrop:x + lcrop, y - lcrop:y + lcrop, z - lcrop:z + lcrop, :] = predArray[
                                                                                                      x - lcrop:x + lcrop,
                                                                                                      y - lcrop:y + lcrop,
                                                                                                      z - lcrop:z + lcrop,
